...
"""A wrapper to loop over all the 2.3m data and apply run_daophot.sh. Needs the user to check the subtracted images to check the psf fitting is good"""
...
if type_choice=='s':
	...
	if not os.path.exists(info_fpath):
		...
		print info_fpath
		sys.exit()
	fin_fpath = working_dir + '/man_wrapper_standard_mags.tab'
	...
	if not os.path.exists(info_fpath):
		...
		print info_fpath
		sys.exit()
	fin_fpath = working_dir + '/man_wrapper_cs_mags.tab'
...
for sheetname in info:
	...
	for line in data_arr:
		...
		#TESTING ONLY USING NIGHT1####################################################################




		if line['Night']!='Night1': continue
		




		#while the star subtraction isn't good enough
		subtract_choice='n'
		while subtract_choice =='n':
			...
			root_coords_path = standard_dir + '/pick_stars.tab'
			coords_path = standard_dir + '/'+str(frame_num)+'_pick_stars.tab'
			
			#if the pick stars file for the frame doesn't exist, copy the base one
			if not os.path.exists(coords_path):
				shutil.copyfile(root_coords_path, coords_path)
				
			with open(coords_path, 'r') as f:
				psf_stars = [l for l in f]
			...
			#open the subtracted sdf file using gaia
			sub_fpath = os.getcwd()+'/'+str(frame_num)+'s.sdf'
			print 'PSF reference stars filepath: '
			print coords_path
			sp.call( [ "/star/bin/gaia/gaia.sh", sub_fpath ] )
			
					
			subtract_choice = None
			while subtract_choice!='y' and subtract_choice!='n':
				subtract_choice = raw_input('Is the subtraction good? (y/n)')
				change_stars = raw_input('Change the pick_stars file and press and key to continue')
